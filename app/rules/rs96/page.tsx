...
const RULES: { t: string; d: string }[] = [
  ...
  {
    t: "일본(JP) 시총 필터",
    d: "시총 상위 20%. 두 데이터 source 결합: ① finance.yahoo.co.jp 직접 시총(도요타·소니·MUFG·키엔스·닌텐도 등 99개, 정확값) ② yfinance 발행주식수 × 그 주차 종가(1,785개, 추정값). 통합 49% 커버. 실제 컷오프는 약 1,174억엔으로 미국 시장 수준의 대형주만 남김(Phase A 9,568건 → 1,312건). 시총 데이터 없는 일본 종목 ~50%는 자동 제외되어 일부 정상 대형주가 빠질 가능성 있음. 향후 다른 source 도입 시 100% 커버 목표.",
  },
  {
    t: "RS96+ 의 의미와 한계",

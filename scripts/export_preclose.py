...
def build(target_date, live):
    ...
    px, nmap, mmap, target, live = sc.prepare(cfg, target_date, live)
...
def main():
    ...
    target, rows = build(target_date, live)
    if args.dry_run:
        print(f"[dry-run] {target} 신규 동시호가 후보 {len(rows)}건")
        for r in rows[:30]:
